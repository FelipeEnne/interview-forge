...
export function CodingChallenge({
  ...
  return (
    <main className={styles.container}>
      <Link className={styles.backLink} href={backLink.href}>{t("backToChallenges")}</Link>
      ...
      <section aria-labelledby="requirements-title">
        <h2 id="requirements-title" className={styles.sectionTitle}>{t("requirements")}</h2>
        <ul className={styles.list}>
          {challenge.requirements.map((requirement, index) => (
            <li key={`${challenge.id}-requirement-${index}`}>{localize(requirement)}</li>
          ))}
        </ul>
      </section>
      <section aria-labelledby="starter-code-title">
        <h2 id="starter-code-title" className={styles.sectionTitle}>{t("starterCode")}</h2>
        <pre className={styles.code}><code>{challenge.starterCode}</code></pre>
      </section>
      <section aria-labelledby="review-checklist-title">
        <h2 id="review-checklist-title" className={styles.sectionTitle}>{t("reviewChecklist")}</h2>
        <ul className={styles.list}>
