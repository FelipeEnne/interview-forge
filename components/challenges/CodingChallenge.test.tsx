...
const challenge: CodingChallengeData = {
  ...
  category: "hooks",
  prompt: { en: "Implement a toggle hook.", pt: "Implemente um hook de toggle." },
  requirements: [{ en: "Expose a toggle function.", pt: "Exponha uma função toggle." }],
  starterCode: "export function useToggle() {}",
  referenceSolution: "export function useToggle() { return [false, () => {}]; }",
...
describe("CodingChallenge", () => {
  it("renders a generic challenge and hides its solution", () => {
    render(<CodingChallenge challenge={challenge} category={category} backLink={{ href: "/topics/react/challenges" }} />);
    expect(screen.getByRole("heading", { name: "Build useToggle" })).toBeInTheDocument();
    ...
    expect(screen.queryByText(challenge.referenceSolution)).not.toBeInTheDocument();
  });
  ...
  it("localizes content and category supplied by the topic", async () => {
    renderWithLocale(<CodingChallenge challenge={challenge} category={category} backLink={{ href: "/topics/react/challenges" }} />, "pt");
    expect(await screen.findByRole("heading", { name: "Crie useToggle" })).toBeInTheDocument();
    expect(screen.getByText("Implemente um hook de toggle.")).toBeInTheDocument();
  });
});
